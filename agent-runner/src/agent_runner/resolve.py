...
def resolve_route(
    ...
    Raises `RouteError`, by name, for each of the ways it can fail: the gateway
    does not offer the SDK, no adapter is built for the SDK, the variable the
    route names is unset or empty, or a ``stored`` route was given no value.

    ``stored`` is spec 46 §7's auth: the HOST keeps the key, encrypted by the
    operating system's own keystore, and hands the plaintext in here. It is a
    parameter rather than something this function can look up, because looking
    it up would mean this package could read a keystore — and spec 42 §3.2 is
    that the library holds no such thing.
    """
    ...
    if route.auth == "stored":
        if not stored:
            raise RouteError(f"The {sdk} route on '{gateway.name}' needs a key, and the host supplied none.")
        token = stored
