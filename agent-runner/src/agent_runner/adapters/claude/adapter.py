...
from ...attribution import anthropic_custom_headers, attribution_headers
from ...events import (
    Completed,
    Denial,
    DeniedEvent,
...
class ClaudeAdapter:
    ...
    def _env(
        ...
        **Nothing at all for the SDK's own endpoint.** `auth: inherit` with no
        ...
        # Spec 45 §6 — which REX thread is spending this, for the gateway's own
        # dashboards. Set only when there is something to say: an empty
        # `ANTHROPIC_CUSTOM_HEADERS` is a header block with one blank line in
        # it, and the CLI has no reason to be handed one.
        if headers:
            env["ANTHROPIC_CUSTOM_HEADERS"] = anthropic_custom_headers(headers)
