...
export function configureConnection(db: Db): void {
  ...
  // One writer again (spec 09 §3.1), so this is belt rather than braces — but a
  // handle that waits for a lock costs nothing and a handle that throws
  // SQLITE_BUSY immediately is the worst kind of intermittent bug.
