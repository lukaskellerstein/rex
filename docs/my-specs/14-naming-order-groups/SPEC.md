...
every group's menu. (Spec 30 §13.8 moved the foot's button up to the make row
under the header, beside *New comment* — the two commands that make something
are in one place now.) A new group is created collapsed-open, at the end of its
...
| §7.2's *"an indent per level of nesting, and nothing else changes"* | **a `tree(1)` connector per level, and a hard break where the folders end** — `treeCells` in `shared/commentTree.ts`, `RowTree` in `Sidebar.tsx`, `.rex-branch-*`, `.rex-out`, `.rex-loose`; the step grew 14px → 20px, and the indent moved from the row's contents onto `.rex-drop` so the connector can be positioned from zero | reported 2026-09-02: the reviewer could not see where a folder's comments stopped and the comments in no folder began. 14px is nothing beside a 340px card, and the list's own 8px gap put the same space at that boundary as between two comments in one folder. A plain vertical rail was the first answer and was reported again — *"visually not compelling, it is still mixing the things together"*: a line beside a block of cards is decoration until it visibly **enters** each one. So the tree is drawn the way `tree` draws one — `├` while the folder has rows to come, a round-cornered `└` on its last, a blank column where an ancestor already closed — and the boundary is a **3px bar, edge to edge through the pane's own padding, with 60px of clear space**. Two quieter versions were rejected before it: a 1px rule inset with the cards reads as one more card border, and a card border is the most common line on this panel |
| — | **`treeCells` and `startsLooseBlock` live in `shared/`, beside `flattenRows`** | they are the same kind of thing: pure answers about the walk, and both are wrong in ways that look deliberate rather than broken. `test/comments.spec.ts` covers them and caught the two rules that mattered. **The connector** asks whether the row has a later *sibling*, not whether any deeper row follows — so a folder whose last row is a nested folder closes with `└` on its own line. **The boundary** asks what is directly above, not whether the depth changed: a collapsed or empty folder puts a depth-0 header straight above a depth-0 comment, nothing steps out, and the first rule drew nothing at exactly the moment the boundary was hardest to see. That is how it shipped wrong on 2026-09-02 |
| — | **the connector's classes are `.rex-branch-*`, not `.rex-tree-*`** | `.rex-tree` is already the explorer's file tree. A second one by that name inherited its `flex-direction: column`, every cell collapsed to zero height, and only the horizontal ticks drew — a bug that looks exactly like "the CSS did not load" |
