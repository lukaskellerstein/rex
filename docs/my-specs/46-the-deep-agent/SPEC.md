...
> `agent-gateway/src/agent_gateway/adapters/deep_agents/`, and it runs
...
Spec 42 built the seam, spec 43 gave it gateways, and specs 44 and 45 filled
in the two SDKs that, like Claude's, wrap a CLI. This spec fills in
`deep-agents`, and it is the one that tests whether the seam is really about
agents and not about child processes.
...
already has a row for it (spec 43 §12). What this spec adds is one adapter
directory, three dependencies, and the proofs that its ASK cannot write.
...
The library adds three packages, each a `uv add` in `agent-gateway/`, and each
...
The Deep Agents column of spec 43 §2.3's catalogue is `{host}/v1` for both
`litellm` and `envoy`, and the adapter reaches it with `ChatOpenAI`, in exactly
the samples' shape:
...
1. **It is what the reviewer's samples do.** Every lesson builds
   ...
   the provider.
2. **It is measured.** Spec 43 §15.1 records the engine streaming incremental
   `tool_calls` deltas through this route — exactly what `ChatOpenAI` consumes,
   and the thing most local models get wrong.

One class for both gateways is also one fewer thing to get wrong.
...
|:--|:--|
| SDK / account default | no URL: the provider's own variable, resolved by main (§4.2). With a URL: **refused** — a generated OpenAI-compatible client has no login convention to inherit, exactly as spec 45 §4 refuses it for OpenCode |
...
Deep Agents has no model catalogue and no probe. A route offers the models
typed into it — spec 43 §4.3 — and that includes `Original`: its
`deep-agents` route carries a typed list of `provider:model` ids.
...
## 10. What changes in specs 43 and 44

Three lines:

1. The agent control (spec 44) gains a fourth row, from `list_sdks()`.
2. The Deep Agents column of the catalogue (spec 43 §2.3) is filled: `{host}/v1`
   for `litellm` and `envoy`.
...
```text
agent-gateway/src/agent_gateway/adapters/deep_agents/
...
does not special-case. Tests are `agent-gateway/tests/test_deep_agents_*.py`:
...
| `LITELLM_BASE_URL` default `http://localhost:4000/v1`, alias `unsloth-gemma-26b` | `model.py:41-51` | a route's `base_url` and `models`; note the course's LiteLLM is a **different instance** from `ai-gateway`'s on 24000 |
...
### 12.3 The gateway side is already measured

Spec 43 §15.1: LiteLLM `POST :24000/v1/chat/completions` 200; the engine
streams `tool_calls` deltas with fragmented `arguments`; `unsloth-26b` spends
tokens on a reasoning block before it answers, so the client's `max_tokens`
must be generous. Envoy: `ai-gateway/envoy/tests/common.py:186-187` drives
`base_url="http://localhost:26000/v1"` with `api_key="no-key-needed"` for chat
completions — the OpenAI chat route through Envoy is exercised by the gateway's
own tests.

Two base URLs back a Deep Agents route on this machine:

| Base URL | Model id | Auth |
|:--|:--|:--|
| `http://localhost:24000/v1` | `unsloth-26b` | Environment variable · `AI_GATEWAY_KEY` |
| `http://localhost:26000/v1` | `unsloth-26b` | No authentication |

Spike against LiteLLM first; it is the route spec 43's milestone 0 has already
driven.
...
5. Whether `usage_metadata` is populated through LiteLLM and through Envoy.
...
- A shell for the deep agent — `LocalShellBackend` or any sandbox backend.
  Nothing in 2.0 has a second layer in front of `execute`.
...
`http://localhost:24000/v1` with `AI_GATEWAY_KEY` and `unsloth-26b`. The three
...
*Gate:* no UI work before all eight items of §12.4 are recorded here.
...
through LiteLLM and through Envoy, stops, runs at the same time as a Claude
route, and the §8.1 hostile prompt changes nothing.
