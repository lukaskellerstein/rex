...
from agent_gateway import ORIGINAL_GATEWAY, resolve_route, validate_base_url
from agent_gateway.resolve import RouteError
from agent_gateway.types import AgentGateway, GatewayRoute


def gateway_with(route: GatewayRoute) -> AgentGateway:
    return AgentGateway(id="g", name="Gateway", kind="custom", routes={"claude-agent": route})
...
def test_an_sdk_with_no_adapter_is_refused_by_name() -> None:
    with pytest.raises(RouteError, match="No adapter for 'codex'"):
        resolve_route(ORIGINAL_GATEWAY, "codex", {})


def test_a_gateway_that_does_not_offer_the_sdk_is_refused_by_name() -> None:
    empty = AgentGateway(id="g", name="Gateway", kind="custom", routes={})
