...
```bash
uv add --editable /path/to/rex/agent-gateway
```
...
```bash
uv run python -m agent_gateway.protocol --schema     > schema.json
uv run python -m agent_gateway.protocol --typescript > ../src/shared/agent-protocol.ts
uv run python -m agent_gateway.protocol --catalogue  > catalogue.json
```
